...
export default function Menu(props: { buttonClassName?: string }) {
  ...
  const { isRecording, isEcRecording, isVideoAvailable, isVonageVideoAvailable } = useIsRecording();

  const { room, vonageSession, setIsBackgroundSelectionOpen } = useVideoContext();

  const anchorRef = useRef<HTMLButtonElement>(null);
  const { flipCameraDisabled, toggleFacingMode, flipCameraSupported } = useFlipCameraToggle();

  const downloadRecords = useCallback(async () => {
    if (!room) return
      const urls = await getRecords(room.sid)
  ...
  const downloadVonageRecord = useCallback(async () => {
    if (!archive) return
      const url = await getVonageRecord(archive)
      if (typeof url === 'string') {
        downloadVideo(url)
      }
  }, [archive, getVonageRecord])
  ...
  return (
    <>
      ...
      >
        ...
        {roomType !== 'peer-to-peer' && roomType !== 'go' && !isEcRecording && (
          <MenuItem
            disabled={isFetching}
            onClick={() => {
              setMenuOpen(false);
        ...
        {!isRecording && (
          <MenuItem
            ...
            onClick={() => {
              ...
              if (isEcRecording) {
                if (archive && ec) {
                  stopEcRecording(ec, archive);
                }
              } else {
                if (vonageSession) {
                  startEcRecording(vonageSession.sessionId, window.location.href).then(({ ecId, archiveId }) => {
                    setEc(ecId);
            ...
            data-ec-cy-recording-button
          >
            <IconContainer>{isEcRecording ? <StopRecordingIcon /> : <StartRecordingIcon />}</IconContainer>
            <Typography variant="body1">{isEcRecording ? 'Stop' : 'Start'} Vonage EC Recording</Typography>
          </MenuItem>
        )}
        <MenuItem
          onClick={() => {
        ...
        </MenuItem>
        {
          ...
          >
            ...
            <Typography variant="body1">Download Record</Typography>
          </MenuItem>
          )
        }
        {
          ...
          >
            ...
            <Typography variant="body1">Download Vonage Record</Typography>
          </MenuItem>
          )
        }
