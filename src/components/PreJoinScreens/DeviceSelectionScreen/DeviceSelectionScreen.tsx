...
export default function DeviceSelectionScreen({ name, roomName, setStep }: DeviceSelectionScreenProps) {
  ...
  const { getToken, getVonageCredential, isFetching, isKrispEnabled, isKrispInstalled } = useAppState();
  ...
  const handleJoin = () => {
    ...
    // Vonage
    getVonageCredential(roomName).then(({ apiKey, sessionId, token }) => {
      vonageConnect(apiKey, sessionId, token);
    });
  };

  if (isFetching || isConnecting || isVonageConnecting) {
    return (
