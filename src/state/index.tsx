...
export interface StateContextType {
  ...
  getVonageCredential(room: string): Promise<{ apiKey: string; sessionId: string; token: string }>;
  ...
  startEcRecording(sessionId: string, url: string): Promise<{ ecId: string; archiveId: string }>;
  stopEcRecording(ecId: string, archiveId: string): Promise<object>;
  getRecords(roomSid: string): Promise<string[]>;
  getVonageRecord(archiveId: string): Promise<string | object>;
...
export default function AppStateProvider(props: React.PropsWithChildren<{}>) {
  ...
  if (process.env.REACT_APP_SET_AUTH === 'firebase') {
    ...
    contextValue = {
      ...
      getVonageCredential: async room_name => {
      ...
      startEcRecording: async (sessionId, url) => {
        const endpoint = process.env.REACT_APP_VONAGE_ARCHIVE_ENDPOINT || '/ecStartRecording';
        ...
        return fetch(endpoint, {
          ...
          .then(async res => {
            ...
            if (!res.ok) {
              const recordingError = new Error(
                jsonResponse.error?.message || 'There was an error starting EC recording'
              );
              recordingError.code = jsonResponse.error?.code;
              return Promise.reject(recordingError);
            }
      ...
      stopEcRecording: async (ecId, archiveId) => {
        const endpoint = process.env.REACT_APP_VONAGE_ARCHIVE_ENDPOINT || '/ecStopRecording';
      ...
      getVonageRecord: async archiveId => {
  ...
  const getVonageCredential: StateContextType['getVonageCredential'] = room => {
        console.log("get vonage token")

    setIsFetching(true);
    return contextValue
      .getVonageCredential(room)
      .then(res => {
        setIsFetching(false);
        return res;
      })
  ...
  const startEcRecording: StateContextType['startEcRecording'] = (sessionId, url) => {
    setIsFetching(true);
    return contextValue
      .startEcRecording(sessionId, url)
      .then(res => {
        setIsFetching(false);
        return res;
      })
  ...
  const stopEcRecording: StateContextType['stopEcRecording'] = (ecId, archiveId) => {
    setIsFetching(true);
    return contextValue
      .stopEcRecording(ecId, archiveId)
      .then(res => {
        setIsFetching(false);
        return res;
      })
